...
// ─────────────────────────────────────────────
// SuccessModal — for teacher quizzes with hidden results
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// Main: QuizPage (MCQ only — AI & teacher quiz)
// ─────────────────────────────────────────────
export default function QuizPage({ config, setPage, setResults }) {
  // NOTE: For descriptive teacher_assignment quizzes, route to <AssignmentQuizPage> instead.
  // This page handles: config.type === "ai" | "teacher" (MCQ questions only)

  const [qs, setQs]           = useState([]);
  const [cur, setCur]         = useState(0);
  const [ans, setAns]         = useState({});   // { [questionIdx]: optionIdx }
  ...
  const doSubmit = useCallback(async () => {
    ...
    try {
      ...
      // Teacher quiz with hidden results
    ...
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", background: "var(--bg)" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <span style={{ fontSize: 16, fontWeight: 700, color: timerCol, fontVariantNumeric: "tabular-nums" }}>
        ...
        <div style={{ flex: 1, padding: "26px 38px", overflowY: "auto" }}>
          <div style={{ maxWidth: 700, margin: "0 auto" }}>
            ...
            <div style={card({ padding: 20, marginBottom: 16 })}>
              ...
              <p style={{ fontSize: 15, color: "var(--white)", lineHeight: 1.7, margin: 0 }}>
                {q.question_text}
              </p>
            </div>
